...
public class ProcessingContext {
    ...
    public ProcessingContextState State { get; set; }

    public SequenceBase SequenceBase { get; }

    private MemoryStackFrameManager MemoryStackFrame { get; }

    public MemoryStack? CurrentFrame => MemoryStackFrame.CurrentFrame;

    public SequenceNode? CurrentNode { get; set; }
    ...
    public ProcessingContext? Child { get; private set; }
    ...
    public ProcessingContextStep RegisterNextExecution(NodeSlot slot) {
        if (slot.Node is ActionNode == false)
            throw new InvalidOperationException(
                "ProcessingContext.RegisterNextExecution() : the node is not an ActionNode"
            );
