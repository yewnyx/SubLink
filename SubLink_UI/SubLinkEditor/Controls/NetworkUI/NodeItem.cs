...
namespace tech.sublink.SubLinkEditor.Controls.NetworkUI;

/// <summary>
/// This is a UI element that represents a network/flow-chart node.
/// </summary>
internal class NodeItem : ListBoxItem {
    ...
    internal void RightMouseDownSelectionLogic()  {
        if (ParentNetworkView.SelectedNodes.Count == 0) {
            // Nothing already selected, select the item.
            IsSelected = true;
        } else if (ParentNetworkView.SelectedNodes.Contains(this) || ParentNetworkView.SelectedNodes.Contains(DataContext)) {
            // Item is already selected, do nothing.
        } else {
    ...
    protected override void OnMouseMove(MouseEventArgs e) {
        ...
        if (_isDragging) {
            ...
            if (dragDistance > DragThreshold) {
                ...
                NodeDragStartedEventArgs eventArgs = new(NodeDragStartedEvent, this, new[] { this });
    ...
    internal void LeftMouseUpSelectionLogic() {
        if (_isLeftMouseAndControlDown) {
            ...
            if (ParentNetworkView.SelectedNodes.Count == 1 &&
                (ParentNetworkView.SelectedNode == this || ParentNetworkView.SelectedNode == DataContext)
            ) {
                // The item that was clicked is already the only selected item.  Don't need to do anything.
            } else {
